...
class AudioController {
    ...
    func config() {
        guard !configurated else { return }
        bindSongToPlay()
        bindTime()
        configPlayingBar()
        configControlCenter()
    ...
    func bindSongToPlay() {
        selectedSong.asObservable()
            ...
            .observeOn(ConcurrentDispatchQueueScheduler.init(qos: .userInteractive))
            .flatMapLatest {
                return Song.getLink(songId: $0.id)
            }
            .observeOn(ConcurrentDispatchQueueScheduler.init(qos: .userInteractive))
    ...
    func nextSong() {
        ...
        case .off:
            guard var index = self.songs.value.index(where: { $0 == self.selectedSong.value }) else { return }
        ...
        DispatchQueue.global().asyncAfter(deadline: .now() + coolDownDuration) { [weak self] _ in
    ...
    func previousSong() {
        ...
        self.selectedSong.value = self.songs.value[index]
        DispatchQueue.global().asyncAfter(deadline: .now() + coolDownDuration) { [weak self] in
            self?.isCoolingDown = false
        }
    }
